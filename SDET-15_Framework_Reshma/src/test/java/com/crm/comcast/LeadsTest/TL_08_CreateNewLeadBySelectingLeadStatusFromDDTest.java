...
public class TL_08_CreateNewLeadBySelectingLeadStatusFromDDTest {
	...
	public void CreateNewLeadIndustry() throws DocumentException {
		...
		//enter mandatory fields and create lead by selecting Lead status from Dropdown
		driver.findElement(By.name("lastname")).sendKeys("Gudda");
		driver.findElement(By.name("company")).sendKeys("ITServices");
		
		WebElement selectdropdown = driver.findElement(By.name("leadstatus"));
		
		Select dropdown = new Select(selectdropdown);
		dropdown.selectByValue("Qualified");
		
		driver.findElement(By.cssSelector("input[title='Save [Alt+S]']")).click();
		
		//validate
        String successMsg = driver.findElement(By.xpath("//span[@class='dvHeaderText']")).getText();
        Assert.assertTrue(successMsg.contains("Gudda"));
